...
public:
    ...
    AVLTreeNode<Data, Key>* getRoot() { return root; }
    int treeSize() { return size; }
};


template <class Data, class Key>
AVLTreeNode<Data, Key>* AVLTree<Data, Key>::rotateLeft(AVLTreeNode<Data, Key> *a) {
...
template <class Data, class Key>
AVLTreeNode<Data, Key>* AVLTree<Data, Key>::rotateRight(AVLTreeNode<Data, Key> *b) {
...
template <class Data, class Key>
AVLTreeNode<Data, Key>* AVLTree<Data, Key>::rotateLeftRight(AVLTreeNode<Data, Key> *n) {
    n->left = rotateLeft(n->left);
    return rotateRight(n);
}

template <class Data, class Key>
AVLTreeNode<Data, Key>* AVLTree<Data, Key>::rotateRightLeft(AVLTreeNode<Data, Key> *n) {
    n->right = rotateRight(n->right);
    return rotateLeft(n);
}

template <class Data, class Key>
bool AVLTree<Data, Key>::insert(Data *data, Key *key) {
    if(!data || !key) {
        return false;
    }
    if(isKeyExists(key)) {
...
template <class Data, class Key>
void AVLTree<Data, Key>::remove(AVLTreeNode<Data, Key> *node) {
    if(!root || !node) {
...
template <class Data, class Key>
void AVLTree<Data, Key>::reBalance(AVLTreeNode<Data, Key> *node) {
    setBalance(node);
...
template <class Data, class Key>
void AVLTree<Data, Key>::setBalance(AVLTreeNode<Data, Key> *node) {
    node->balance = getHeight(node->left) - getHeight(node->right);
}

template <class Data, class Key>
int AVLTree<Data, Key>::getHeight(AVLTreeNode<Data, Key> *node) {
    if (!node){
        return -1;
    }
...
template <class Data, class Key>
bool AVLTree<Data, Key>::isKeyExists(Key *key) {
    if(!root) {
        return false;
    }
...
template <class Data, class Key>
AVLTreeNode<Data, Key>* AVLTree<Data, Key>::find(Key *key) {
    if(!key) {
        return nullptr;
    }
